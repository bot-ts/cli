...
import { cwd, fetchGenmap } from "./util"

export async function genFromGenmap(program: Command) {
  const genmap = await fetchGenmap()

  for (const [name, dirname] of Object.entries(genmap)) {
    const templatePath = cwd("templates", name + ".txt")
    const destDir = cwd("src", dirname)
    ...
    while ((match = pattern.exec(template))) {
      params.add(match[1].toLocaleLowerCase())
    }
...
//     await fsp.writeFile(cwd("src", "app.ts"), appLines.join("\n"), "utf8")
